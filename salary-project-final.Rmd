...
## 3.3 Modeling
Now we have selected the model using AIC with transformations, let's construct a model using log transformed response variable.
```{r}
# We first fit a linear model
final_model <- lm(log_salary ~ . + experience_level:company_size, 
                  data = joined_df)


summary(final_model)
```
We check if the assumptions of linear regressions are met through the following Residuals vs Fitted plot and a QQ plot. The assumptions are linearity, independence, homoscedasticity and normality.
```{r}
# TA plot to check linearity, independence, homoscedasticity
...
qqnorm(final_model$residuals, main = "QQ Plot")
qqline(final_model$residuals, col = "red")
```

By using ANOVA, we can verify the overall significance of our model and predictors. 
```{r}
